"""
05_train_probes_nt_v2.py
------------------------
Trains probes on the cached Nucleotide Transformer v2 embeddings for two tasks:
...
convention (dots replaced with underscores), e.g. ``layer.12``
becomes ``layer_12.h5``. The original layer name is preserved in the
file's ``layer_name`` attribute and used as the display label.

NT v2 layer index convention:
  layer.0   = token embedding output
  layer.1   .. layer.32 = transformer block outputs
  layer.32  = last hidden state (equivalent to hidden_states[-1])
...
Usage
-----
  python 05_train_probes_nt_v2.py                          # all cached layers
  python 05_train_probes_nt_v2.py --skip_mlp               # faster (logistic only)
  python 05_train_probes_nt_v2.py --targets phylum         # phylum only
  python 05_train_probes_nt_v2.py --probe_layers layer.32  # single layer
"""
...
# NT v2 has 33 transformer blocks + 1 embedding layer = indices 0..32
NT_NUM_LAYERS = 33
...
def _block_index(layer_name: str) -> int:
    """
    Returns the integer block index for a layer name, used for sorting
    and tick labels.

    'layer.N'  -> N
    'layer_N'  -> N   (safe-filename variant stored in .h5)
    anything else -> very large int (sorts last)
    """
...
def _layer_tick_labels(layers: list[str]) -> list[str]:
    ...
    for name in layers:
        ...
        short.append(f"L{idx}" if idx < 10 ** 9 else name)
    return short
...
def plot_layer_curves_per_target(
    ...
    for ax, metric in zip(axes, ["f1", "auc", "auprc"]):
        ...
        ax.legend()
    fig.suptitle(f"NT v2 2.5B - Classification Probes ({target})")
    plt.tight_layout()
...
def plot_phylo_curves(phylo_df: pd.DataFrame, out_path: Path):
    ...
    ax.set_title("NT v2 2.5B - Phylogenetic Distance Correlation by Layer")
    ax.legend()
    plt.tight_layout()
    fig.savefig(out_path, dpi=150)
    plt.close()
...
def plot_single_layer_summary(
    ...
    fig.suptitle(f"NT v2 2.5B - {layer_name}")
    plt.tight_layout()
    fig.savefig(out_path, dpi=150)
    plt.close()
    print(f"Saved {out_path}")
...
def main():
    ...
    parser.add_argument(
        ...
        help=(
            ...
            f"Valid range: layer.0 (embedding) .. layer.{NT_NUM_LAYERS} (last block)."
        ),
    )
    parser.add_argument(
        "--targets",
        ...
        help=(
            f"Comma-separated classification targets. "
    ...
    if not available:
        raise FileNotFoundError(
            ...
            f"Run 04_extract_embeddings_nt_v2.py first."
        )
    ...
    for layer_name in layer_names:
        ...
        if not np.isfinite(X_train).all():
            ...
            raise ValueError(
                ...
                f"values in the train split — re-run 04_extract_embeddings_nt_v2.py."
            )
